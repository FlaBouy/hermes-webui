...
// Strips markdown, code blocks, MEDIA: paths, raw WebUI URLs,
// filenames/link titles, scores, document-route / UI chrome, and raw
// retrieval payloads so spoken output is answer prose only.
function _stripForTTS(text){
  text=String(text==null?'':text);
  // Document-route / card-title boilerplate (answer body only).
  text=text.replace(/(^|\n)[ \t]*Document links(?:\s+for\s+[“"][^”"]*[”"])?(?:\s*\(sidecar preview\))?\s*:?[ \t]*(?=\n|$)/gi,'$1');
  text=text.replace(/\bDocument links(?:\s+for\s+[“"][^”"]*[”"])?(?:\s*\(sidecar preview\))?\s*:?/gi,' ');
  text=text.replace(/\(\s*score\s*=\s*[-+]?\d*\.?\d+\s*\)/gi,' ');
  text=text.replace(/\b(?:sidecar preview|lan_url|card title|source pill|owner\s*ack)\b\s*:?/gi,' ');
  // Raw retrieval JSON / field dumps (nested match objects first).
  for(let i=0;i<8;i++){
    const next=text.replace(/\{[^{}]*\}/g,(body)=>{
      return /"(?:matches|collection|lan_url|snippet|source|score|url|markdown)"\s*:/i.test(body)?' ':body;
    });
    if(next===text) break;
    text=next;
  }
  text=text.replace(/\b(?:matches|collection|snippet|source|topk|library_only|snippet_chars)\b\s*[:=]\s*/gi,' ');
  if(/"(?:matches|collection)"\s*:/i.test(text) || /\b(?:matches|collection)\b\s*[:=]/i.test(text)){
    return '';
  ...
  // Images + markdown links drop entirely (no filename / link title spoken)
  text=text.replace(/!\[[^\]]*\]\([^)]+\)/g,' ');
  text=text.replace(/\[[^\]]*\]\([^)]+\)/g,' ');
  // Raw absolute URLs and relative WebUI sidecar paths must not be spoken
  text=text.replace(/https?:\/\/[^\s<>\]\)"']+/gi,' ');
  text=text.replace(/\/api\/extensions\/smedley-engineering\/sidecar\/(?:preview|doc)\/[^\s<>\]\)"']*/gi,' ');
  // Bare corpus filenames left after link stripping
  text=text.replace(/\b[\w./\\-]+\.(?:pdf|docx?|xlsx?|pptx?|txt|md)\b/gi,' ');
...
function _playElevenLabsTts(text, btn, opts){
  opts=opts||{};
  if(btn) btn.dataset.speaking='1';
  _ttsSpeaking=true;
  ...
  const body={text:text, engine:'elevenlabs'};
  if(opts.voice_id) body.voice_id=String(opts.voice_id);
  fetch(new URL('api/tts', document.baseURI || location.href).href, {
    method:'POST',
    headers:{'Content-Type':'application/json'},
...
function autoReadLastAssistant(){
  ...
  if(pref!=='true') return;
  // Ask Jarvis hard-bind: speak spoken_text only via documented Jarvis ElevenLabs voice.
