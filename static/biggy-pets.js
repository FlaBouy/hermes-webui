...
(() => {
  ...
  function render(){
    if(!root)return;syncRuntimes();renderControls();const visible=state.instances.filter(instance=>instance.enabled&&runtimes.get(instance.instanceId)?.loaded).length;button.classList.toggle('is-active',visible>0);button.setAttribute('aria-label',`Animated object controls: ${visible} visible`);
