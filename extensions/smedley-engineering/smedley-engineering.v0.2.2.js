(() => {
  ...
  function retrieveFromComposer(note){const composer=document.getElementById('msg'),q=(composer&&composer.value||'').trim();if(!q){note.textContent='Type the engineering question in the Smedley message box first.';return;}note.textContent='Searching the engineering library…';const filter=searchScope==='library'?{library_only:true}:currentProject&&searchScope!=='library'?{project:currentProject,scope:searchScope}:{};proxyJson('/rag/retrieve',{method:'POST',body:JSON.stringify({query:q,topk:8,snippet_chars:900,filter})}).then((data)=>{composer.value='';composer.dispatchEvent(new Event('input',{bubbles:true}));sendThroughHermes(buildGroundedPrompt(q,data.matches));note.textContent=`${(data.matches||[]).length} source excerpt(s) sent to Smedley.`;}).catch((error)=>{note.textContent=`Retrieval failed: ${error.message||error}`;});}
