"""A.R.G.U.S. bridge: loopback allowlist, honest offline/error, no secrets."""
...
import api.argus_bridge as bridge
...
def test_non_loopback_base_is_rejected():
    jb = bridge.ArgusBridge(base_url="http://192.168.0.15:4719")
...
def test_health_offline_on_connection_refused(monkeypatch):
    ...
    jb = bridge.ArgusBridge(base_url="http://127.0.0.1:4719")
    payload, status = jb.health()
    assert status == 200
...
def test_chat_empty_question():
    payload, status = bridge.ArgusBridge().chat("   ")
    assert status == 400
