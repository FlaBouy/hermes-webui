...
- The first isolated cockpit-object POC lives at `/static/argus-cockpit-pet-poc.html`. It reuses the accepted Orb art, menu geometry, A.R.G.U.S. label, and model/state readout; only the bounded red center eye follows the pointer. The production Biggy shell does not import or register this component. Its twelve buttons emit inert stable action IDs for later parity wiring after visual acceptance.
