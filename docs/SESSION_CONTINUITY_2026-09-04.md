...
- Local animated objects are selectable from an ANI control at the bottom of the PA right rail. They support on/off, resizing, dragging, keyboard movement, per-object saved placement, slow/random animation, and reduced-motion behavior without an LLM, voice call, or network polling.
- The Objects manager supports any number of independent on-screen instances, including multiple copies of the same object. The catalog and on-screen-object selectors are separate; each instance owns visibility, size, position, and timing, with selected and global show/hide/remove controls. Existing single-object browser state migrates automatically.
- The accepted Orb/menu/readout renderer is now the production cockpit Object. It is permanently mounted on glass; ANI may move, resize, and center it but cannot hide or remove it. Other animated Objects retain independent show/hide/remove controls.
- The production shell imports the shared renderer, maps its twelve stable action IDs to the existing hidden Hermes controls, mirrors active controls back into the Orb, and drives model, online, tool, error, and speech state through attributes. A selected menu action produces a solid white connection; idle remains dotted.
- The standalone reviewer page remains inert at `/static/argus-cockpit-pet-poc.html`; its adapter makes no production calls.
