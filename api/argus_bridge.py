"""Same-origin A.R.G.U.S. bridge for the Biggy GUI.

The browser talks only to Biggy (`/api/biggy/v6/*`). This adapter proxies a
narrow allowlist to the local A.R.G.U.S. service. Upstream must be loopback.
...
_PROFILE_CONFIG = Path.home() / ".hermes" / "profiles" / "biggy" / "argus-bridge.json"
_REPO_LOCAL_CONFIG = Path(__file__).resolve().parents[1] / "argus-bridge.local.json"
_LEGACY_PROFILE_CONFIG = Path.home() / ".hermes" / "profiles" / "biggy" / "jarvis-v6-bridge.json"
_LEGACY_REPO_LOCAL_CONFIG = Path(__file__).resolve().parents[1] / "jarvis-v6-bridge.local.json"


class ArgusBridge:
    """Loopback-only proxy onto A.R.G.U.S. health/status and /chat."""
    ...
    def _request(
        ...
        *,
        ...
        try:
            ...
            logger.exception("argus bridge request failed")
            return 0, None, "A.R.G.U.S. bridge request failed", "error"


def load_bridge_config() -> dict[str, Any]:
    merged: dict[str, Any] = {}
    # Read the retired filenames first, then let canonical Argus config win.
    for path in (_LEGACY_PROFILE_CONFIG, _LEGACY_REPO_LOCAL_CONFIG, _PROFILE_CONFIG, _REPO_LOCAL_CONFIG):
        data = _read_json_file(path)
        if isinstance(data, dict):
            merged.update(data)
    env_url = (os.environ.get("ARGUS_BASE_URL") or os.environ.get("JARVIS_V6_BASE_URL") or "").strip()
    if env_url:
...
def _read_json_file(path: Path) -> dict[str, Any] | None:
    try:
        ...
        logger.warning("argus bridge config is not valid JSON: %s", path)
        return None
...
def _validate_loopback_base(base: str) -> str | None:
    try:
        ...
        return "invalid A.R.G.U.S. base URL"
    if parsed.scheme not in {"http", "https"}:
        return "A.R.G.U.S. base URL must be http(s)"
    host = (parsed.hostname or "").strip().lower()
    if host not in {"127.0.0.1", "localhost", "::1"}:
        return "A.R.G.U.S. base URL must be loopback"
    if parsed.username or parsed.password:
        return "A.R.G.U.S. base URL must not include credentials"
    if parsed.path not in {"", "/"}:
        return "A.R.G.U.S. base URL must not include a path"
    return None
...
# Transitional import alias for older tests/extensions. New runtime code uses
# ``ArgusBridge`` through ``api.argus_bridge``.
JarvisBridge = ArgusBridge
