"""Read-only Mapbox-backed trip planning for Jarvis II PA."""
...
def _public_place_fallback(query: str) -> dict[str, Any] | None:
    ...
    request = urllib.request.Request(
        ...
        headers={"Accept": "application/json", "User-Agent": "Jarvis-II-PA/1.0 (read-only place verification)"},
    )
    try:
...
def _recommendation(category: str, title: str, notice: str, features: list[dict[str, Any]], location_label: str) -> dict[str, Any]:
    ...
    return {"schema": "jarvis.recommendation_view_model.v1", "emitted_by": "Jarvis II PA Tool", "category": category, "title": f"{title} near {location_label}", "available": bool(options), "reason": None if options else "NO_PUBLIC_POIS_FOUND", "options": options[:5], "source": _SOURCE, "notice": notice}
...
def resolve_travel(*, destination: str, include_lodging: bool = True) -> dict[str, Any]:
    ...
    return {"ok": True, "schema": "jarvis.pa.travel_evidence.v1", "source": _SOURCE, "destination": place, "recommendation_view_model": _recommendation("lodging", "Lodging", "Public POIs only; no rates, availability, or booking status.", features, place["label"])}
...
def plan_trip(*, origin: str, destination: str) -> dict[str, Any]:
    ...
    map_view_model = {"schema": "jarvis.map_view_model.v1", "emitted_by": "Jarvis II PA Tool", "available": True, "origin": origin_place, "destination": destination_place, "route": {"distance_m": route.get("distance"), "duration_min": round(float(route.get("duration") or 0) / 60, 2), "mode": "driving", "geometry": geometry}, "navigation": {"google_maps_url": f"https://www.google.com/maps/dir/?api=1&origin={encoded_origin}&destination={encoded_destination}&travelmode=driving", "waze_url": f"https://www.waze.com/ul?q={encoded_destination}&navigate=yes"}, "source": _SOURCE, "notice": "Mapbox route and temporary public POIs only; verify road conditions and availability before travel."}
    return {"ok": True, "schema": "jarvis.pa.trip_plan.v1", "source": _SOURCE, "origin": origin_place, "destination": destination_place, "map_view_model": map_view_model, "recommendation_view_models": models, "notice": "Read-only planning evidence. A.R.G.U.S. cannot book, purchase, reserve, or verify live availability."}
